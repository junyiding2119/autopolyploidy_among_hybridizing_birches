...
WorkDir=/data_analysis/costataeEvolution/Result/02s_PolyAncestor_main_analysis_two_ash
...
mkdir MSA Iteration_cos Iteration_ashTZZ Iteration_ashEDL Iteration_bugg Iteration_len

bcftools view -s Alnus_glutinosa_ERR2103731,lenta_ERR2026272,nana_ERR2026268,occidentalis_ERR2026271,pendula_ERR2026097,populifolia_ERR2026269,EDL21-009-platy,XYB21-001-mcalli,TZZ78,EDL028,CKX18-006,QSLC10,${poly} -O z -o ${poly}.snp.NO_VARIATION.QC.vcf.gz /data_analysis/costataeEvolution/Result/01s_PolyAncestor_variant_calling_two_ash/07all.snp.NO_VARIATION.QC.nocall0.1.vcf.gz 
...
for k in ./MSA/*.clean.fasta; do
   python3 /data_analysis/costataeEvolution/Result/02PolyAncestor_main_analysis/01s_polarizeTETRA.py $k 13 "CKX18-006" ${poly}
...
/data_analysis/costataeEvolution/Result/02s_PolyAncestor_main_analysis_two_ash/02_obs_polyAncestor_query.get_number_informative_MSA.sh ${ref} ${poly}
...
ref=cos
for k in ./MSA/*.clean.fasta; do
   python3 /data_analysis/costataeEvolution/Result/02PolyAncestor_main_analysis/01s_polarizeTETRA.py $k 13 "QSLC10" ${poly}
   #echo MSAstep $k finished
done
...
ref=ashTZZ
for k in ./MSA/*.clean.fasta; do
   python3 /data_analysis/costataeEvolution/Result/02PolyAncestor_main_analysis/01s_polarizeTETRA.py $k 13 "TZZ78" ${poly}
   #echo MSAstep $k finished
done
...
ref=len
for k in ./MSA/*.clean.fasta; do
   python3 /data_analysis/costataeEvolution/Result/02PolyAncestor_main_analysis/01s_polarizeTETRA.py $k 13 "lenta_ERR2026272" ${poly}
   #echo MSAstep $k finished
done
...
ref=ashEDL
for k in ./MSA/*.clean.fasta; do
   python3 /data_analysis/costataeEvolution/Result/02PolyAncestor_main_analysis/01s_polarizeTETRA.py $k 13 "EDL028" ${poly}
   #echo MSAstep $k finished
done
